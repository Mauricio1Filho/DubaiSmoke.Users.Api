...
namespace DubaiSmoke.Users.Infrastructure.Repositories.MySql
{
    ...
    public class AddressRepository : IAddressRepository
    {
        ...
        public async Task<bool> DeleteAsync(long id) => await _unitOfWork.Connection.ExecuteAndReturnBoolAsync(
                @"DELETE FROM address WHERE ID = @Id", new { id });
        ...
        public async Task<long> InsertAsync(AddressEntity item) => await _unitOfWork.Connection.QueryFirstOrDefaultAsync<long>(
                @"INSERT INTO address (ID_USER, TXT_ADDRESS, NR_ADDRESS, TXT_COMPLEMENT, TXT_POSTAL_CODE, DT_CREATED, DT_UPDATED, HASH_CODE)
                  Values (@UserId, @AddressName, @AddressNumber, @AddressComplement,@PostalCode, @CreatedAt, @UpdatedAt, @HashCode);
        ...
        public async Task<bool> UpdateAsync(AddressEntity item) => await _unitOfWork.Connection.ExecuteAndReturnBoolAsync(
